...
- How to get the environment running again (`npm ci && npm run build && npm test`)
- Anything that is NOT reproducible from the repo — a Pages setting changed in
  the GitHub UI, a DNS record, a repo rename. None of it is in git, and it is
  the class of state a fresh session cannot discover by reading code.
